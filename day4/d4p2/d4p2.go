...
// wrong 27270
func Run() {
	measurements, err := os.Open("day4/d4p2/" + file + ".txt")
	if err != nil {
		log.Fatal("open: ", err)
	}
	...
	var tableWon = make([]bool, len(tables))
...
func allWon(won []bool) bool {
	for _, isTableWon := range won {
		if !isTableWon {
			return false
		}
	}
